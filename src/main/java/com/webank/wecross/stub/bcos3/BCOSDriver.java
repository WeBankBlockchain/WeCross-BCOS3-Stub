...
public class BCOSDriver implements Driver {
            ...
            if (subType == TransactionParams.SUB_TYPE.SEND_TX_BY_PROXY) {
              ...
              if (transactionParams
                ...
                Tuple4<String, String, String, byte[]> constantCallProxyFunctionInput =
                    FunctionUtility.getConstantCallProxyFunctionInput(transactionParams.getData());
                abi = Hex.toHexString(constantCallProxyFunctionInput.getValue4());
              } else {
                Tuple2<String, byte[]> sendTransactionProxyFunctionInput =
                    FunctionUtility.getConstantCallFunctionInput(transactionParams.getData());
                    ...
                    contractCodecJsonWrapper
                        .encode(
                            ...
                            Objects.nonNull(transactionRequest.getArgs())
                                ? Arrays.asList(transactionRequest.getArgs())
                                : Arrays.asList())
                        .encode(isWasm));
      ...
      asyncBfsService.queryABI(
          ...
          (queryABIException, abi) -> {
            try {
              ...
              List<ABIDefinition> functions = contractABIDefinition.getFunctions().get(method);
                  ...
                  connectionResponse -> {
                    try {
                      ...
                      if (Objects.equals(
                        ...
                        // decode 130
                        String output = callOutput.getOutput().substring(130);
                        transactionResponse.setResult(
                            contractCodecJsonWrapper
                                .decode(outputObj, Hex.decode(output), isWasm)
                                .toArray(new String[0]));
                      } else {
                        transactionResponse.setErrorCode(BCOSStatusCode.CallNotSuccessStatus);
                        ...
                        if (booleanStringTuple2.getValue1()) {
                          ...
                          transactionResponse.setMessage(
                              TransactionReceiptStatus.getStatusMessage(
                                      callOutput.getStatus(), "Unknown error")
                                  .getMessage());
                        }
                      }
              ...
              logger.warn(" e: ", bse);
              callback.onTransactionResponse(
                  new TransactionException(bse.getErrorCode(), bse.getMessage()), null);
            } catch (Exception e) {
              logger.warn(" e: ", e);
              callback.onTransactionResponse(
                  new TransactionException(BCOSStatusCode.UnclassifiedError, e.getMessage()), null);
            }
          });
    ...
    asyncBfsService.queryABI(
        ...
        (queryABIException, abi) -> {
          try {
            ...
            if (!Objects.isNull(args)) {
              ABIObject encodedObj = contractCodecJsonWrapper.encode(inputObj, Arrays.asList(args));
              encodedArgs = encodedObj.encode(isWasm);
            }
            ...
            Request req =
                Request.newRequest(
                    BCOSRequestType.SEND_TRANSACTION, objectMapper.writeValueAsBytes(transaction));
        ...
        response -> {
          try {
            ...
            if (response.getErrorCode() != BCOSStatusCode.Success) {
              throw new BCOSStubException(response.getErrorCode(), response.getErrorMessage());
            }
            ...
            if (receipt.isStatusOK()) {
                      ...
                      asyncRequestTransactionProof(
                          ...
                          (exception, proof) -> {
                            if (Objects.nonNull(exception)) {
                              callback.onTransactionResponse(
                                  new TransactionException(
                                      BCOSStatusCode.HandleGetTransactionProofFailed,
              ...
              transactionResponse.setErrorCode(BCOSStatusCode.SendTransactionNotSuccessStatus);
              if (receipt.getStatus() == TransactionReceiptStatus.RevertInstruction.getCode()) {
                Tuple2<Boolean, String> booleanStringTuple2 =
                    RevertMessage.tryParserRevertMessage(receipt.getStatus(), receipt.getOutput());
                if (booleanStringTuple2.getValue1()) {
                  transactionResponse.setMessage(booleanStringTuple2.getValue2());
                } else {
                ...
                transactionResponse.setMessage(
                    TransactionReceiptStatus.getStatusMessage(receipt.getStatus(), "Unknown error")
                        .getMessage());
              }
              callback.onTransactionResponse(null, transactionResponse);
            }
  ...
  public void assembleTransactionResponse(
      TransactionReceipt receipt, Callback callback, List<ABIDefinition> functions) {
    try {
      TransactionResponse transactionResponse = new TransactionResponse();
      ...
      transactionResponse.setErrorCode(BCOSStatusCode.Success);
      transactionResponse.setMessage(BCOSStatusCode.getStatusMessage(BCOSStatusCode.Success));
      callback.onTransactionResponse(null, transactionResponse);
      if (logger.isDebugEnabled()) {
        logger.debug(" hash: {}, response: {}", receipt.getTransactionHash(), transactionResponse);
      }
    } catch (Exception e) {
      logger.warn(" e: ", e);
      callback.onTransactionResponse(
  ...
  public void asyncGetBlock(
    ...
    Request request =
        Request.newRequest(
            BCOSRequestType.GET_BLOCK_BY_NUMBER, BigInteger.valueOf(blockNumber).toByteArray());
          ...
          if (isVerified) {
            MerkleValidation.verifyTransactionProofWithCallback(
                ...
                verifyException -> {
                  ...
                  assembleTransaction(
                      transactionHash, transaction, transactionReceipt, connection, callback);
                },
                cryptoSuite);
          } else {
            assembleTransaction(
  ...
  private void assembleTransaction(
    ...
    try {
      ...
      transaction.getTransactionResponse().setBlockNumber(receipt.getBlockNumber().longValue());
      ...
      asyncBfsService.queryABI(
          ...
          (queryABIException, abi) -> {
            if (Objects.nonNull(queryABIException)) {
              logger.error(
                  "Query abi failed, transactionHash: {}, e: ", transactionHash, queryABIException);
              callback.onResponse(null, transaction);
              return;
            }
            ...
            try {
              List<String> inputParams =
                  contractCodecJsonWrapper.decode(inputObject, Hex.decode(finalInput), isWasm);
  ...
  private void asyncRequestTransactionProof(
    ...
    Request request = Request.newRequest(BCOSRequestType.GET_TRANSACTION_PROOF, transactionHash);
    connection.asyncSend(
        request,
        response -> {
          try {
            ...
            if (response.getErrorCode() != BCOSStatusCode.Success) {
              callback.onResponse(
                  new BCOSStubException(response.getErrorCode(), response.getErrorMessage()), null);
              return;
            }
            ...
            callback.onResponse(
                new BCOSStubException(BCOSStatusCode.UnclassifiedError, e.getMessage()), null);
          }
        });
  }
  ...
  public void checkTransactionRequest(TransactionContext context, TransactionRequest request)
    ...
    if (Objects.isNull(context)) {
      throw new BCOSStubException(BCOSStatusCode.InvalidParameter, "TransactionContext is null");
    }

    if (Objects.isNull(request)) {
      throw new BCOSStubException(BCOSStatusCode.InvalidParameter, "TransactionRequest is null");
    }

    if (Objects.isNull(context.getBlockManager())) {
      throw new BCOSStubException(BCOSStatusCode.InvalidParameter, "BlockHeaderManager is null");
    }
